...
import type { SidebarProps, ProjectInfo } from '../types/types';
...
const Sidebar: React.FC<SidebarProps> = ({
  ...
  return (
    <div className={`w-full md:w-[280px] bg-white bg-opacity-50 backdrop-blur-xl rounded-2xl p-8 md:p-10 m-0 md:m-4 shadow-xl border border-white border-opacity-60 transition-all duration-300 ease-in-out ${className}`}>
      {/* プロジェクト情報セクション */}
      <div className="mb-6">
        <div 
        ...
        >
          ...
          {projectInfo.description && (
            <p className="text-sm text-gray-600 mt-1 ml-9">{projectInfo.description}</p>
          )}
        </div>
      </div>
